...
class TileRenderer {
    constructor({ width, height, gradient }) {
        ...
        this.gradientRange = gradient;
        this.gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
        if (!this.gl) {
            throw Error('Failed to get WebGL context');
        }

        const modifiedFragmentShader = elevationGradientFrag.replace(/GRADIENT_STOP_COUNT/g, this.gradientRange.length.toString());
    ...
    render(
        ...
        maskDim,
        tileDimension,
        gradientMinElevation,
        gradientMaxElevation,
        opacityMinElevation,
        majorContour,
        minorContour,
        gradOpacity) {

        const { gl, canvas, program, canvasElement, gradientRange } = this;
        ...
        const gradient = gradientRange
        const gradientColors = []
        gradient.forEach(({ color: { red, green, blue, alpha } }) => {
            gradientColors.push(red);
            gradientColors.push(green);
            gradientColors.push(blue);
            gradientColors.push(alpha);
        });
        
        const gradientHeights = gradient.map(({ value }) => value)
        ...
        const gradientHeightLocation = gl.getUniformLocation(program, 'u_gradientHeights');
