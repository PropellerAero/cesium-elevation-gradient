...
class TileRenderer {
    constructor({ createCanvas, width, height, gradientStops, gradientAmount, hillshadeAmount, contourAmount, majorContour, minorContour, contourOpacityThreshold, useSlope, contourColor }) {
        const { canvas, gl } = getCanvasAndWebGL(width, height, createCanvas)
    ...
    render(
        ...
        return cloneCanvas(canvasElement, this.createCanvas)
    }
}

const createCanvas = (width, height, createCanvasFactory = null) => {
    if (createCanvasFactory) {
        return createCanvasFactory(width, height)
    }
...
const getCanvasAndWebGL = (width, height, createCanvasFactory = null) => {
    const key = JSON.stringify({ width, height })
    if (!canvases[key]) {
        const canvas = createCanvas(width, height, createCanvasFactory)
...
const cloneCanvas = (oldCanvas, createCanvasFactory = null) => {
    const newCanvas = createCanvas(oldCanvas.width, oldCanvas.height, createCanvasFactory);
