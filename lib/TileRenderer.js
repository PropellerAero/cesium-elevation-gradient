...
import { createCanvas } from '@propelleraero/isomorphic-canvas';
...
class TileRenderer {
    ...
    render(
        ...
        return cloneCanvas(canvasElement, createCanvas)
    }
}

const createCanvasFromCreator = (width, height, creator = null) => {
    if (creator) {
...
const getCanvasAndWebGL = (width, height, creator = null) => {
    const key = JSON.stringify({ width, height })
    if (!canvases[key]) {
        const canvas = createCanvasFromCreator(width, height, creator)
...
const cloneCanvas = (oldCanvas, creator = null) => {
    const newCanvas = createCanvasFromCreator(oldCanvas.width, oldCanvas.height, creator);
