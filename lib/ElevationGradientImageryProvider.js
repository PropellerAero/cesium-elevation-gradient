...
class ElevationGradientImageryProvider {
    ...
    requestImage = (x, y, tileLevel) => {
        ...
        const { gridSize, maskSize, minorContour, majorContour, fontSize, formatContourLabel } = this
        ...
        return when.all([maskPromise, valuePromise]).then(
            ([maskSamples, valueSamples]) => {
                ...
                return renderContourLabels(canvas, valueSamples, maskSamples, majorContour, minorContour, fontSize, formatContourLabel);
            }
        ).otherwise(() => {
            return this.blankCanvas;
        })
