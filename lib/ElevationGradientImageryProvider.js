...
class ElevationGradientImageryProvider {
    constructor({
        ...
        this.blankCanvasPromise = when.resolve(createCanvas ? createCanvas(this.tileSize, this.tileSize) : makeBlankCanvas(this.tileSize))
    ...
    requestImage = (x, y, tileLevel) => {
        ...
        if (response) {
            return Promise.resolve(handleRequest(response))
        }
