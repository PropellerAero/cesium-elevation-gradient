...
class ElevationGradientImageryProvider {
    constructor({
        ...
        this.tilingScheme = tilingScheme || new GeographicTilingScheme({ ellipsoid })
        this.color = color
    ...
    requestImage = (x, y, tileLevel) => {
        const { gridSize, maskSize, minorContour, majorContour, fontSize, formatContourLabel, gradientAmount, contourAmount } = this
        ...
        return when.all([maskPromise, valuePromise]).then(
            ([maskSamples, valueSamples]) => {
                ...
                return contourAmount > 0.01 ? renderContourLabels(canvas, valueSamples, maskSamples, majorContour, minorContour, fontSize, formatContourLabel, shouldRenderContourLabel) : canvas
            },
        ).otherwise(() => this.blankCanvas)
    }
...
const rectangleToCartographicGrid = (rectangle, divisionsX, divisionsY) => range(divisionsX * divisionsY).map((i) => {
