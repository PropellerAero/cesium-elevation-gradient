...
class ElevationGradientImageryProvider {
    ...
    requestImage = (x, y, tileLevel) => {
        ...
        const resolveBlankCanvas = () => when.resolve(this.blankCanvas)

        if (tileLevel < this.minimumTileLevel) {
            return resolveBlankCanvas()
        }
        ...
        if (this.maskSampler) {
            ...
            maskPromise = when(this.maskSampler(maskSampleLocations))
        } else {
