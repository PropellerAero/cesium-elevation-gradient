        ...
        return when.all([maskPromise, valuePromise]).then(
            ([maskSamples, valueSamples]) => {
                const canvas = this.tileRenderer.render(
                    ...
                    maskSize,
                    tileDimension,
                    this.gradientMinElevation,
                    this.gradientMaxElevation,
                    this.opacityMinElevation,
                    majorContour,
                    minorContour,
                    this.gradOpacity
                )

                return renderContourLabels(canvas, valueSamples, majorContour, minorContour, fontSize);
            }
        ).otherwise(() => {
            return this.blankCanvas;
        })
